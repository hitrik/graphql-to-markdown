...
function renderObject(type, options) {
  ...
  fields.forEach((field) => {
    printer('<tr>')
    printer(
      `<td colspan="2" valign="middle"><strong>${field.name}</strong>${
        field.isDeprecated ? ' ⚠️' : ''
      }</td>`
    )
    printer(`<td valign="middle">${renderType(field.type, { getTypeURL })}</td>`)
  ...
  if (enums.length) {
    ...
    enums.forEach((type) => {
      ...
      type.enumValues.forEach((value) => {
        printer('<tr>')
        printer(
          `<td valign="middle"><strong>${value.name}</strong>${
  ...
  if (unions.length) {
    ...
    unions.forEach((type) => {
      ...
      type.possibleTypes.forEach((objType) => {
        ...
        printer('<tr>')
        printer(
          `<td valign="middle"><strong>${renderType(objType, {
            getTypeURL,
          })}</strong></td>`
        )
        if (desc) {
